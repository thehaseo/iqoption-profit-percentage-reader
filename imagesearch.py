...
the top left corner coordinates of the element if found as an array [x,y] or [-1,-1] if not
'''
def imagesearch(image, precision=0.9, save=None):
...
def imagematch():
    img1 = cv2.imread("image/ganancia-actual-big-font.png", cv2.IMREAD_GRAYSCALE)          # queryImage
    im = pyautogui.screenshot()
    img_bgr = np.array(im)
    img2 = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
